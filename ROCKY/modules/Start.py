...
import psutil
import config
from ROCKY import _boot_
from ROCKY import get_readable_time
from ROCKY import ROCKY, mongo
...
from ROCKY.database.users import get_served_users, add_served_user
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup, Message, CallbackQuery
from ROCKY.modules.helpers import (
    START,
    START_BOT,
    PNG_BTN,
    CLOSE_BTN,
...
@ROCKY.on_message(filters.new_chat_members)
        ...
        for member in message.new_chat_members:
            ...
            if member.id == ROCKY.id:
                try:
                    ...
                    pass
                try:
                    invitelink = await ROCKY.export_chat_invite_link(message.chat.id)
                ...
                try:
                    groups_photo = await ROCKY.download_media(
                ...
                count = await ROCKY.get_chat_members_count(chat.id)
                chats = len(await get_served_chats())
...
async def start(_, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        if m.chat.photo:
            try:
                userss_photo = await ROCKY.download_media(m.chat.photo.big_file_id)
                await umm.delete()
                if userss_photo:
        ...
        await m.reply_photo(photo=chat_photo, caption=START.format(ROCKY.mention or "can't mention", users, chats, UP), reply_markup=InlineKeyboardMarkup(START_BOT))
        await add_served_user(m.chat.id)
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(f"{m.chat.first_name}", user_id=m.chat.id)]])
        await ROCKY.send_photo(int(OWNER_ID), photo=chat_photo, caption=f"{m.from_user.mention} ʜᴀs sᴛᴀʀᴛᴇᴅ ʙᴏᴛ. \n\n**ɴᴀᴍᴇ :** {m.chat.first_name}\n**ᴜsᴇʀɴᴀᴍᴇ :** @{m.chat.username}\n**ɪᴅ :** {m.chat.id}\n\n**ᴛᴏᴛᴀʟ ᴜsᴇʀs :** {users}", reply_markup=keyboard)
...
@ROCKY.on_cmd("help")
async def help(client: ROCKY, m: Message):
    if m.chat.type == ChatType.PRIVATE:
...
async def ping(_, message: Message):
    ...
    await loda.edit_text(
        text=f"нey вαву!!\n{ROCKY.name} ᴄʜᴀᴛʙᴏᴛ ιѕ alιve 🥀 αnd worĸιng ғιne wιтн a pιng oғ\n\n**➥** `{ms}` ms\n**➲ ᴄᴘᴜ:** {CPU}\n**➲ ʀᴀᴍ:** {RAM}\n**➲ ᴅɪsᴋ:** {DISK}\n**➲ ᴜᴘᴛɪᴍᴇ »** {UP}\n\n<b>||**๏ мαdє ωιтн ❣️ ву [ᴀɴᴊᴀʟɪ ʀᴏʙᴏᴛ](https://t.me/{OWNER_USERNAME}) **||</b>",
...
@ROCKY.on_message(filters.command("statsts"))
                ...
                for chat in chats:
                    ...
                    try:
                        if broadcast_type == "reply":
                            m = await ROCKY.forward_messages(
                                chat_id, message.chat.id, [broadcast_content.id]
                            )
                        else:
                            m = await ROCKY.send_message(
